...
| RB | R1–R3 PROMOTED | Robustness: fail requests, not the server, under physical VRAM pressure (R1–R5 below) | [RB report](logs/rb_robustness_3090_2026-09-16.md): churn crash reproducer → 38 evict-and-retry, 0 errors; R2 turns allocation failures into HTTP 503 with the server alive; flags-on RC4′ checks on the promoted profile pass (A 64/64 to 231K, 215K prime served, TG/TTFT unchanged) with a sole-owner floor bypass; flags on in `serve-3090.sh` since 2026-09-16 (`d0866e7c...`); R4 not started, R5 drafted |
