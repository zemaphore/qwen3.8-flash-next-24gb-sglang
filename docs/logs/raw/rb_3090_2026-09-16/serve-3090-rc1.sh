...
CTL="${CTL:-/root/qwen3.8-flash-next-24gb-sglang/docs/logs/raw/rc1_3090_2026-09-16/elastic-rc1.ctl}"
SRVLOG="${SRVLOG:-/root/qwen3.8-flash-next-24gb-sglang/docs/logs/raw/rc1_3090_2026-09-16/server-rc1.log}"
...
setsid systemd-run --user --scope --unit="sglang-$(date +%s)" -p MemoryMax=56G \
  env PATH="$CU/bin:$VENV/bin:$PATH" CUDA_HOME="$CU" \
      ...
      SGLANG_KV_LAZY_TOKENS="${SGLANG_3090_LAZY_TOKENS:-131072}" \
  ...
  "$VENV/bin/python3" -m sglang.launch_server \
    ...
    --model-path "$MODEL" --max-total-tokens "${SGLANG_3090_MAX_TOKENS:-131072}" --context-length "${SGLANG_3090_CONTEXT:-262144}" \
    --kv-cache-dtype int8ring_int4 --attention-backend triton --max-mamba-cache-size "${SGLANG_3090_MAMBA_SIZE:-8}" \
