#!/usr/bin/env bash
# Qwen3.8-Flash-Next-int2 serving on RTX 3090 (sm_86): main-based port, promoted.
# Tree /root/quant/sglang-main (SGLang main b02e16a895 + series-main), venv
# /root/quant/venv-sglang-main. Reproduces the promoted 4-slot radix profile
# (230K pool, presence placement S184, R1-R3 on) with prefill graphs explicitly
# disabled (main no longer auto-disables them for multimodal models).
# Rollback: /root/quant/serve-3090-frozen-rollback.sh (frozen flags-on control,
# sha256 d0866e7c...); snapshot under docs/logs/raw/promotion_3090_2026-09-16/.
set -euo pipefail
...
setsid systemd-run --user --scope --unit="sglang-main-$(date +%s)" -p MemoryMax=56G \
  ...
  "$VENV/bin/python3" -m sglang.launch_server \
    ...
    --served-model-name "${SERVED_MODEL_NAME:-qwen38-flash-230K}" \
    --model-path "$MODEL" --max-total-tokens 235520 --context-length 235520 \
    --kv-cache-dtype int8ring_int4 --attention-backend triton --max-mamba-cache-size 4 \
