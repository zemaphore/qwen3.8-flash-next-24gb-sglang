#!/usr/bin/env python3
"""TG3: candidate-vs-baseline timing on the two held-out prompts.

256 greedy tokens, EOS ignored, one excluded warmup + three measured per prompt,
identical order/procedure. Uses the tg_baseline sample machinery.

  python3 tg3_timing.py --prompts prompts.json --boot-label candidate --out-dir t_cand
"""
...
ORDER = ("code_held", "reasoning_held")
...
def main() -> None:
    ...
    for key in ORDER:
        ...
        for n in range(a.warmups + a.measured):
            ...
            recs.append(rec)
            print(f"  {a.boot_label} {key:14s} #{n} "
